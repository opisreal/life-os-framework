...
- **亏损加仓**：调 `parse_trades.detect_add_on_losing(trades 窗口行)`（同向加仓且劣于持仓均价，同秒部分成交聚类为一次；2026-07-03 起 trades.csv 由 API 自动落盘，不再"无法判定"）——报事件数 + 最重一例（symbol/时间/加仓价 vs 均价）
...
**kill 判据（2026-07-03 W27 补裁定版）**：自动周报连续 4 期（W28 起算）未获任何回应（setup 标签/出入金登记/周复盘触发/任何 finance 指令均算回应）→ 砍掉 finance loop，不再有下一次形态转换。旧"主动跑 loop"判据已于 W26 命中并随人肉形态废除，见 `05_finance/README.md`。
