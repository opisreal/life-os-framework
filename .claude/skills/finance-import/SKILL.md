---
name: finance-import
description: 理财模块导入型唯一写入口。当用户说"导入成交""拉一下交易""上传交易""贴 Bitget CSV""发截图记交易""导入流水""记一下消费"或输入 /finance import 时使用；输入只含权益数（"记权益""报权益"）走仅权益轻路径。MVP-1.6 步骤②全自动已上线（launchd 周日自动拉取，本 skill 管手动导入/补拉/suspected 确认）；Bitget API 直拉为首选（--source api，可带 --equity 自动权益行），手动 CSV 降级为兜底；成交明细 fills API 轨道已校准（--kind fills → trades.csv）；支出分支 MVP-2 起启用。
---
...
> **MVP-1.6 步骤②（全自动）已上线（2026-07-03）**：launchd 周日 20:00 跑 `_tools/weekly-auto-pull.sh`——API 拉已平仓+权益 → 写表 → commit → 飞书通知"材料已备好"；**任何失败降级执行催促探针** `weekly-submit-reminder.sh`（人肉 CSV 路径永远可用）。本 skill 的手动触发场景收敛为：周中主动导入 / 补拉 / suspected 确认 / CSV 兜底（API 失败或超 3 个月回溯窗口的历史补录）/ 截图路径。**API 直拉仍是首选**（`--source api`，字段映射已 9/9 真仓对账锁定）；手动与自动共用 `import_closed_pnl.py` 同一去重/落盘管道。fills（成交明细）已于 2026-07-03 真实样本校准接入（`--kind fills` → `risk/trades.csv`，自动拉取含此轨道）。
...
- **Bitget 成交明细（fills）：API 轨道已校准**（2026-07-03 真实样本 236 条实拉，见 `exchange-schemas.md` fills 段）——`--source api --kind fills` → `risk/trades.csv`，tradeId 主键去重；**posMode 非 hedge_mode 或组合未知整行拒绝出 warning**（单向持仓未实测，不猜）。**CSV 路径仍未校准**（从未拿到真实导出），`--source csv --kind fills` 显式拒绝。
